    ...
    for m, zlow, zhigh in [('.', -1, 1)]:
        ...
        for i in x1s:
            if i >= incs.sector1  and (x1s[n]**2)/(incs.b_ellips1**2) + (y1s[n]**2)/(incs.a_ellips1**2) <= 1:
                ...
                b = y1s[n]
                if a < 0:
                    x1.append(((a**2+b**2)**0.5*np.cos(np.pi+incs.alpha+np.arctan(b/a))+incs.X1)/incs.miu)
                    y1.append(((a**2+b**2)**0.5*np.sin(np.pi+incs.alpha+np.arctan(b/a))+incs.Y1)/incs.miu)
                    z1.append((z1s[n]+y1s[n]+incs.Z1)/incs.miu)
                    n += 1
                else:
                    x1.append(((a**2+b**2)**0.5*np.cos(incs.alpha+np.arctan(b/a))+incs.X1)/incs.miu)
                    y1.append(((a**2+b**2)**0.5*np.sin(incs.alpha+np.arctan(b/a))+incs.Y1)/incs.miu)
                    z1.append((z1s[n]+y1s[n]+incs.Z1)/incs.miu)
                    n += 1
            else:
                n += 1
        ...
        for i in x2s:
            if x2s[n]**2 + y2s[n]**2 <= incs.Radius:
                ...
                z2.append((z2s[n]+y2s[n]+incs.Z2)/incs.miu)
                n += 1
            else:
                n += 1
        n = k
        ...
        for i in x3s:
            if y3s[n]/(incs.katetx3-x3s[n]) <= incs.katety3/incs.katetx3:
                ...
                z3.append((z3s[n]+y3s[n]-incs.Z3)/incs.miu)
                n += 1
            else:
                n += 1
        n = k
        ...
        for i in x5s:
            if i >= incs.sector5 and (x5s[n]**2)/(incs.b_ellips5**2) + (y5s[n]**2)/(incs.a_ellips5**2) <= 1:
                ...
                b = y5s[n]
                if a < 0:
                    x5.append(((a**2+b**2)**0.5*np.cos(incs.alpha+np.pi+np.arctan(b/a))+incs.X5)/incs.miu)
                    y5.append(((a**2+b**2)**0.5*np.sin(incs.alpha+np.pi+np.arctan(b/a))+incs.Y5)/incs.miu)
                    z5.append((z5s[n]+y5s[n]+incs.Z5)/incs.miu)
                    n += 1
                else:                   
                    x5.append(((a**2+b**2)**0.5*np.cos(incs.alpha+np.arctan(b/a))+incs.X5)/incs.miu)
                    y5.append(((a**2+b**2)**0.5*np.sin(incs.alpha+np.arctan(b/a))+incs.Y5)/incs.miu)
                    z5.append((z5s[n]+y5s[n]+incs.Z5)/incs.miu)
                    n += 1
            else:
                n += 1
        ...
        for i in x6s:
            if y6s[n]/(incs.katetx6-x6s[n]) <= incs.katety6/incs.katetx6:
                ...
                z6.append((z6s[n]+y6s[n]+incs.Z6)/incs.miu)
                n += 1
            else:
                n += 1
        n = k
        ...
        for i in x7s:
            if y7s[n]/x7s[n] <= incs.katety7/incs.katetx7:
                ...
                z7.append((z7s[n]+y7s[n]+incs.Z7)/incs.miu)
                n += 1
            else:
                n += 1

        ax.scatter(x1, y1, z1, color='black', s=incs.s)
        ax.scatter(x2, y2, z2, color='black', s=incs.s)
